...
<table border='0' cellpadding='0' cellspacing='0' role='presentation' style='width: 100%;font-family:Arial, Helvetica, sans-serif;border:2px <?php echo esc_attr( $wps_etmfw_border_type . ' ' . $wps_etmfw_border_color ); ?>' id="wps_new_template_border">
	<tbody>
		<tr>
			<td style="width:70%;">
				<div style='color:#fff;border-radius: 15px;padding: 20px;background-image:url(<?php echo esc_url( $wps_image_att_etmfw_demo ); ?>);background-size: 140% 120%;background-position:center center;background-repeat: no-repeat;overflow:hidden;'>
					<div style="background-image: url(<?php echo esc_url( $wps_etmfw_logo_url ); ?>); height: 40px; background-size: contain; background-position: left; background-repeat: no-repeat"></div>
		...
		<tr>
			<td colspan='2' style='padding: 20px;background: #fff;'>
				<h4 style='color:#000;font-weight:bold;font-size:18px;margin:0 0 10px;letter-spacing:0.5px;line-height:1;'><?php echo esc_html__( 'Details', 'event-tickets-manager-for-woocommerce' ); ?></h4>
		...
		<tr>
			<td colspan='2'>
				<div style='border-radius:15px;padding: 20px;background: #cecece4f;'>
					...
					<p style='color:#1e1e1e;font-size:14px;letter-spacing:0.5px;line-height:1.5;'>
					<?php
					if ( '' != $body ) {
						echo wp_kses(
							$body,
