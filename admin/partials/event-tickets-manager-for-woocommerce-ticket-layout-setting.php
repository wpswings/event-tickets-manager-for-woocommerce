...
if ( isset( $_POST['wps_etmfw_new_layout_setting_save'] ) ) {
	...
	if ( wp_verify_nonce( $wps_verify_nonce_form, 'wps_layout_nonce_verify' ) ) {
		...
		// update_option('m1',$_POST['wps_etmfw_background_image']);
	}
}
...
<input type="hidden" name="wps_layout_value" value="<?php echo esc_html( wp_create_nonce( 'wps_layout_nonce_verify' ) ); ?>"/>
				...
				<!--1st Section for the design setting.-->
				<!-- <div class="wps_etmfw_table wps_upsell_table--border wps_etmfw_custom_template_settings "> -->
				<!-- <div class="wps_etmfw_offer_sections"><?php //esc_html_e( 'Template Header', 'upsell-order-bump-offer-for-woocommerce' ); ?></div> -->
				<!-- <table class="form-table wps_etmfw_creation_setting">
				<tbody> -->
				<!-- Border style start. -->
				<!-- <tr valign="top">
					<th>This is a header</th>
					<td>This is the body tage below header</td>
				</tr>
				<tr valign="top">
					<th>This is a header</th>
					<td>This is the body tage below header</td>
				...
				<tr valign="top">
					...
					echo wp_kses( wc_help_tip( $attribute_description ), $allowed_html );
					?>
					<input type="text" name="product_section_price_text_color" class="wps_etmfw_colorpicker wps_etmfw_select_ticket_background" value="blue <?php //echo ! empty( $wps_upsell_bumps_list[ $wps_upsell_bump_id ]['design_css']['product_section_price_text_color'] ) ? esc_html( $wps_upsell_bumps_list[ $wps_upsell_bump_id ]['design_css']['product_section_price_text_color'] ) : ''; ?>">
					</td>
				</tr>
				...
				<tr valign="top">
					...
					echo wp_kses( wc_help_tip( $attribute_description ), $allowed_html );
					?>
					<input type="range" min="100" value="<?php //echo esc_html( $wps_upsell_bumps_list[ $wps_upsell_bump_id ]['design_css']['top_vertical_spacing'] ); ?>"  max="200" value="" name='top_vertical_spacing' class="wps_etmfw_logo_size_slider" />
					<span class="wps_etmfw_logo_size_slider_span" ><?php echo '0'.'px';//echo esc_html( ! empty( $wps_upsell_bumps_list[ $wps_upsell_bump_id ]['design_css']['top_vertical_spacing'] ) ? esc_html( $wps_upsell_bumps_list[ $wps_upsell_bump_id ]['design_css']['top_vertical_spacing'] . 'px' ) : '0px' ); ?></span>
					</td>
				</tr>
				...
				<tr valign="top">
					...
					echo wp_kses( wc_help_tip( $attribute_description ), $allowed_html );
					?>
					<input type="range" min="100" value="<?php //echo esc_html( $wps_upsell_bumps_list[ $wps_upsell_bump_id ]['design_css']['top_vertical_spacing'] ); ?>"  max="220" value="" name='top_vertical_spacing' class="wps_etmfw_qr_size_slider" />
					<span class="wps_etmfw_qr_size_slider_span" ><?php echo '0'.'px';//echo esc_html( ! empty( $wps_upsell_bumps_list[ $wps_upsell_bump_id ]['design_css']['top_vertical_spacing'] ) ? esc_html( $wps_upsell_bumps_list[ $wps_upsell_bump_id ]['design_css']['top_vertical_spacing'] . 'px' ) : '0px' ); ?></span>
					</td>
				</tr>
				...
				<tr valign="top">
					...
					?>
					<?php 
					if ( empty( get_option('m1') ) ) {

					// $image_attributes[0] - Image URL.
					// $image_attributes[1] - Image width.
					// $image_attributes[2] - Image height.
					$image_attributes = wp_get_attachment_image_src( '272', 'thumbnail' );
					?>
