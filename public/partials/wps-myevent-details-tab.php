<?php
				...
				if ( $product instanceof WC_Product && $product->is_type( 'event_ticket_manager' ) ) {
					...
					if ( ! empty( $product ) ) {
						$pro_id = $product->get_id();
						  ...
						  $order_date = $orderObj->get_date_created()->date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ) );
						  $user_id = ( 0 != $orderObj->get_user_id() ) ? '#' . $orderObj->get_user_id() : 'Guest';
						  // echo $start.'<br>';
						  // echo $end.'<br>';
						  // echo $venue.'<br>';
						  // echo $order_date.'<br>';
						  ...
						  $orderme = $orderObj->get_id();
						  $ticket = get_post_meta( $orderObj->get_id(), "event_ticket#$orderme#$item_id", true );
					...
					if ( ! empty( $generated_tickets ) ) {
						foreach ( $generated_tickets as $key => $value ) {
							if ( $ticket == $value['ticket'] ) {
								...
								// echo $orderObj->get_id();

								if ( 'checked_in' === $checkin_status && $orderObj->get_id() == $value['order_id'] ) {
										  $upload_dir_path  = EVENT_TICKETS_MANAGER_FOR_WOOCOMMERCE_UPLOAD_URL . '/events_pdf/events' . $value['order_id'] . $value['ticket'] . '.pdf';
										  // echo '<a href="' . $upload_dir_path . '" target="_blank">Checked In</a>';
										...
										$upload_dir_path  = EVENT_TICKETS_MANAGER_FOR_WOOCOMMERCE_UPLOAD_URL . '/events_pdf/events' . $value['order_id'] . $value['ticket'] . '.pdf';
										// echo '<a href="' . $upload_dir_path . '" target="_blank">Pending</a>';
					...
					$order_received_url = wc_get_endpoint_url( 'order-received', $orderObj->get_id(), wc_get_checkout_url() );
					$order_received_url = add_query_arg( 'key', $orderObj->get_order_key(), $order_received_url );
					...
					$event_attendees_details[] = array(
						...
						'order'             => '<a href="' . esc_url( $order_received_url ) . '">#' . $orderObj->get_id() . '</a>',
						'user'              => $user_id,
						'venue'             => $venue,
						'purchase_date'     => $order_date,
						'schedule'          => wps_etmfw_get_date_format( $start ) . '-' . wps_etmfw_get_date_format( $end ),
						'action'            => '<br><a href="' . $upload_dir_path . '" target="_blank">' . esc_html( 'View', 'event-tickets-manager-for-woocommerce' ) . '</a>',
					);
		...
		$wps_myevent_tab_html .= "<table class='woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table'>";
		...
		foreach ( $event_attendees_details as $mks ) {
			...
			// 	$wps_myevent_tab_html .= '<tr><td>' . $mks['event'] . '</td><td>' . $mks['schedule'] . '</td><td>' . $mks['check_in_status'] . '</td><td>' . wc_price( $mks['price'] ) . '</td><td>' . $mks['order'] . $mks['action'] . '</td></tr>';
			// } else {
				$wps_myevent_tab_html .= '<tr><td>' . $mks['event'] . '</td><td>' . $mks['schedule'] . '</td><td>' . $mks['check_in_status'] . '</td><td>' . wc_price( $mks['price'] ) . '</td><td>' . $mks['order'] . '</td></tr>';
			// }
		}
		$wps_myevent_tab_html .= '</table></div>';

		echo $wps_myevent_tab_html;
    } else {
