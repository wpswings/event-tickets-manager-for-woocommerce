...
class Event_Tickets_Manager_For_Woocommerce_Public {
						...
						<div class="mwb-form-group">
							...
							<div class="mwb-form-group__control">
								...
								<div>
									...
									<label for="yes"><?php  _e( 'Yes', 'event-tickets-manager-for-woocommerce'); ?></label>
								</div>
								<div>
									<input type="radio" id="mwb_etmfw_info_radio" name="mwb_etmfw_info_radio" value="no">
									<label for="no"><?php  _e( 'No', 'event-tickets-manager-for-woocommerce'); ?></label>
								</div>
							</div>
						</div>
						<?php
							...
							if( isset( $ticket_number ) ) {
								...
								if( empty($generated_tickets)){
									...
									$generated_tickets[] = array(
								 	...
								 	update_post_meta( $pro_id, "mwb_etmfw_generated_tickets", $generated_tickets );
								} else{
									$generated_tickets[] = array(
										'ticket' => $ticket_number,
										'status' => 'pending',
			...
			case 'yes-no':
				...
				<div class="mwb-edit-form-group" data-id="<?php echo esc_attr( $html_value_label ); ?>">
					...
					<?php if( $mandatory ) : ?>	
						...
						<?php endif; ?>
					<div>
						<input type="radio" id="mwb_etmfw_<?php echo esc_attr( $html_value_label ); ?>" name="mwb_etmfw_<?php echo esc_attr( $html_value_label ); ?>" value="yes" <?php echo esc_html( $user_data_value == 'yes') ? 'checked' : '';?> <?php echo esc_html( $required ) ?>>
						<label for="yes"><?php  esc_html_e( 'Yes', 'event-tickets-manager-for-woocommerce'); ?></label>
					</div>
					<div>
						<input type="radio" id="mwb_etmfw_<?php echo esc_attr( $html_value_label ); ?>" name="mwb_etmfw_<?php echo esc_attr( $html_value_label ); ?>" value="no" <?php  echo esc_html( $user_data_value == 'no') ? 'checked' : '';?>>
	...
	public function mwb_etmfw_make_user_checkin_for_event(){
		if ( isset( $_REQUEST['mwb_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mwb_nonce'] ) ), 'mwb-etmfw-verify-checkin-nonce' ) ) { // WPCS: input var ok, sanitization ok.
			...
			$product_id = sanitize_text_field( wp_unslash( $_REQUEST['for_event'] ) );
			$ticket_num = sanitize_text_field( wp_unslash( $_REQUEST['ticket_num'] ) );
	...
	public function mwb_etmfw_edit_user_info_for_event(){
		if ( isset( $_REQUEST['mwb_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mwb_nonce'] ) ), 'mwb-etmfw-verify-public-nonce' ) ) { // WPCS: input var ok, sanitization ok.
			$response['result'] = false;
			$posted_value = map_deep( wp_unslash( $_REQUEST['form_value'] ), 'sanitize_text_field' );
